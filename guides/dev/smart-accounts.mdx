...
description: 'Create and configure accounts with advanced authentication capabilities on the IXO blockchain using the IXO MultiClient SDK'
---
...
### Composite authenticator types

<AccordionGroup>
  <Accordion title="AllOf" icon="link">
    - **Description:** Requires all sub-authenticators to approve
    - **Logic:** `authenticate(a) && authenticate(b) && ...`
  </Accordion>
  <Accordion title="AnyOf" icon="code-branch">
    - **Description:** Requires at least one sub-authenticator to approve
    - **Logic:** `authenticate(a) || authenticate(b) || ...`
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="MsgAddAuthenticator" icon="user-plus">
    Adds an authenticator to a smart account.

    **Parameters:** `smartAccount: string, authenticator: string`
  </Accordion>

  <Accordion title="MsgRemoveAuthenticator" icon="user-minus">
    Removes an authenticator from a smart account.

    **Parameters:** `smartAccount: string, authenticator: string`
  </Accordion>

  <Accordion title="MsgSendWithAuthenticator" icon="paper-plane">
    Sends a transaction using an authenticator.

    **Parameters:** `smartAccount: string, recipient: string, amount: string`
  </Accordion>
</AccordionGroup>
