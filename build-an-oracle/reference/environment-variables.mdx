...
- **Request-path caches.** The per-user `SqliteSaver` is reused across the two build-time hook calls per turn instead of being rebuilt (and re-running `setup()`); the agent builder reads prior build-time checkpoints via a light path that skips the message-history join (O(1) instead of O(history)); and the Memory Engine `userContext` cache is keyed by `roomId` instead of `sessionId`, so a fresh session for the same room reuses the cached context.
