...
<AccordionGroup>
  ...
  <Accordion title="getMiddlewares(ctx)" icon="filter">
    ...
    LangChain `AgentMiddleware` instances inserted after the four always-on framework middlewares (tool-validation, retry, page-context, safety-guardrail). Plugin middlewares run in topological dependency order across plugins.
  </Accordion>
