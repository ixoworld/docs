...
<AccordionGroup>
  ...
  <Accordion title="shared" icon="share-nodes">
    Read accessors for state owned by other plugins (registered via `getSharedState()`). Typed via declaration merging on the `SharedAccessors` interface — see [Plugin shared state guide](/build-an-oracle/build/plugin-recipes/share-state).
  </Accordion>
