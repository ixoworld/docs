...
- Most other fields use last-write-wins or "merge if present" semantics; consult the source for the exact reducer when authoring middlewares that mutate state.

Plugin middlewares that need to update state should return a partial state object from the hook. The runtime applies it through the reducer pipeline.
