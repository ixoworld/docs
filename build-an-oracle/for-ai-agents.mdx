...
This page is for AI tools (Cursor, Claude Code, Codex, …) scaffolding a QiForge oracle. Humans should read [Quickstart](/build-an-oracle/quickstart) and [Build a plugin](/build-an-oracle/build/write-a-plugin) instead.
...
- [`/build-an-oracle/build/plugin-recipes/add-a-tool`](/build-an-oracle/build/plugin-recipes/add-a-tool) — one recipe per capability.
