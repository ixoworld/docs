...
graph TB
    ...
    subgraph Runtime["@ixo/oracle-runtime"]
        ...
        Bundled["14 bundled plugins<br/>memory · skills · sandbox · editor · agui · portal · firecrawl · domain-indexer · composio · slack · credits · user-preferences · tasks · calls"]
...
The framework. Owns the bootstrap (`createOracleApp`), the always-on Nest modules (Sessions, Messages, WebSocket, Secrets, UCAN, Auth, Subscription, Throttler, Health), the per-request agent builder, the four always-on middlewares, the Matrix-backed SQLite checkpointer, and the plugin API.

You never edit this layer. Updates come via `pnpm update @ixo/oracle-runtime`.

### Layer 3 — Bundled plugins

14 plugins shipped inside the runtime package, each independently toggleable via `features`. See the [Plugin catalog](/build-an-oracle/reference/bundled-plugins/overview) for what each one does.
...
| MiddlewareRegistry | All plugin middlewares | Ordered topologically; no names |
...
| `hooks.checkpointerForUser` (advanced) | The four always-on middlewares |
...
<CardGroup cols={2}>
  ...
  <Card title="Build your first oracle" icon="play" href="/build-an-oracle/build/create-oracle-app">
