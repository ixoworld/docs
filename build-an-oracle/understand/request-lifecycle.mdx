...
| **LLM invocation** | Composed prompt + bound tools + state messages go to the model selected by `LLM_PROVIDER`. Plugin middlewares fire (`beforeModel`, `afterModel`, `onError`). |
...
| LLM | `onError` middlewares fire. Default: propagate to client. |
