...
description: "Read-only observability UI for the headless Flow Agent. Operators sign in with their IXO identity via the ixo Auth Hub and drill into per-flow runs and span waterfalls."
icon: "gauge-high"
---
...
There is no PIN prompt, no Matrix vault read, and no QR / WebAuthn step — the Auth Hub returns the operator's signing key directly, so the whole exchange finishes in one round trip.
